...
Production commit e54effa2871a429d071e40884e40c48085a6138c contains exactly two source callers of `/api/send-lead`: `src/app/page.tsx` (`/`) and `src/app/free-property-valuation-singapore/page.tsx` (`/free-property-valuation-singapore`). The latter is a deployed lead-generating page listed in the sitemap (weekly, priority 0.95), not obsolete. Both callers support report unlock, consultation and a later intent update. Both now use the same `homevalue-whatsapp-v1` contract and optional unchecked controls; phone edits/new valuations clear consent and the intent update preserves the original evidence. No other source/public caller was found in production main.

### Review correction validation

Both production callers (`src/app/page.tsx` and `src/app/free-property-valuation-singapore/page.tsx`) now offer the same optional, initially unchecked consent on report unlock and consultation. A source-inventory regression test checks both callers, phone-change clearing, original-evidence forwarding and absence of evidence regeneration in the send helper. All 20 Node tests, TypeScript and the production build passed after this correction. Office's correction is documentation only.

Visual checks passed for both pages' report-unlock and consultation forms at desktop (1440 × 1080; home consultation at the normal desktop viewport) and mobile (390 × 844): checkbox unchecked, full notice and Optional label readable, no horizontal clipping, submit button visible. Home consultation was checked in the running local app; the remaining forms were checked using static fixtures extracted from their exact current JSX with the built production CSS (system-font fallback), without scripts or live data connections. These fixtures validate layout, not end-to-end submission. Screenshots were shown in the review conversation. No production lead was submitted.
